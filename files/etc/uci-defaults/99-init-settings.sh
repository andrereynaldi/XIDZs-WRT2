...
check_status "uci set network.mm=interface" "mm interface created"
check_status "uci set network.mm.proto='modemmanager'" "mm protocol set to modemmanager"
check_status "uci set network.mm.device='/sys/devices/platform/scb/fd500000.pcie/pci0000:00/0000:00:00.0/0000:01:00.0/usb2/2-1'" "mm device set"
check_status "uci set network.mm.apn='internet'" "mm APN set to internet"
check_status "uci set network.mm.auth='none'" "mm authentication set to none"
check_status "uci set network.mm.iptype='ipv4'" "mm IP type set to ipv4"
...
check_status "uci set firewall.@zone[1].network='tethering modem mm'" "Firewall zone configured for TETHERING MODEM"
...
# fix issue
log_status "INFO" "Adding fix issue configuration..."
    ...
    if opkg list-installed | grep -qw "$pkg"; then
        ...
        case "$pkg" in
            luci-app-openclash)
                ...
                check_status "rm -f /root/oc.htm" "remove oc.htm"
                ;;
            luci-app-nikki)
